...
const WelcomeMenu = () => {
    ...
    useEffect(() => {
        const [ firstNamespace ] = location.pathname.split("/").filter( item => item !== "")
        setNamespaceSelected(firstNamespace)
    }, [location])
            ...
            <div className="navbar">
                <div className="container-xl">
                    <ul className="navbar-nav">
                        ...
                        {
                            MENU_CONFIG
                                .map(({ icon, title, namespace}) => {
                                    return <li className={`nav-item  ${namespaceSelected === namespace ? "active" : ""}`}>
                                                <a className={`nav-link`} href={`#${namespace}` || "#"}>
