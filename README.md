...
| Documento | Conteúdo |
| --- | --- |
| [`docs/ARQUITETURA.md`](docs/ARQUITETURA.md) | Módulos, camadas, modelo de rings e fluxo de requisições/roteamento. |
| [`docs/COMPONENTES.md`](docs/COMPONENTES.md) | Catálogo de todos os pacotes (apps, serviços, webservices, painéis, CLIs). |
...
# 2. Instala as aplicações da plataforma
...
Depois de instalado, os serviços são provisionados e iniciados pela CLI `my-services`
a partir dos arquivos em [`provisioning-data/`](provisioning-data). O passo a passo
completo (ordem dos rings, portas, hosts `*.local` e operação no dia a dia) está em
[`docs/GUIA-OPERACAO.md`](docs/GUIA-OPERACAO.md).
</content>
</invoke>
